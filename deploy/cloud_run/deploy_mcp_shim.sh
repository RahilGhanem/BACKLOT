...
# a cloud-hosted BACKLOT before a real ClickHouse cluster + mcp-clickhouse
# server are wired up. Once that's available, point the main deploy's
# MCP_MODE=clickhouse / CLICKHOUSE_MCP_URL at it instead and retire this
# service.
#
...
# meant for the real mcp-clickhouse server, which validates its own
# CLICKHOUSE_MCP_AUTH_TOKEN on its side (unless run with
# CLICKHOUSE_MCP_AUTH_DISABLED=true). --allow-unauthenticated here means
# this deployment is reachable by
